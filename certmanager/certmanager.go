...
// CertManager is an abstraction around trusted root CA stores
...
// NewCertManager returns an initialized CertManager, or an error
...
// by this CertManager
func (cm *CertManager) TrustedCertificateStore() trustmanager.X509Store {
	return cm.trustedCertificateStore
}

// TrustedCAStore returns the CA store being managed by this CertManager
func (cm *CertManager) TrustedCAStore() trustmanager.X509Store {
	return cm.trustedCAStore
}

// AddTrustedCert adds a cert to the trusted certificate store (not the CA
// store)
func (cm *CertManager) AddTrustedCert(cert *x509.Certificate) {
	cm.trustedCertificateStore.AddCert(cert)
}

// AddTrustedCACert adds a cert to the trusted CA certificate store
func (cm *CertManager) AddTrustedCACert(cert *x509.Certificate) {
	cm.trustedCAStore.AddCert(cert)
}
...
*/
func (cm *CertManager) ValidateRoot(root *data.Signed, gun string) error {
	...
	certsForCN, err := cm.trustedCertificateStore.GetCertificatesByCN(gun)
	if err != nil {
	...
	for _, cert := range allValidCerts {
		err := cm.trustedCertificateStore.AddCert(cert)
		if err != nil {
	...
	for certID, cert := range certsToRemove(certsForCN, allValidCerts) {
		...
		err = cm.trustedCertificateStore.RemoveCert(cert)
		if err != nil {
